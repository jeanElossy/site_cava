...
- **Frontend** sur Vercel. [vercel.json](vercel.json) fait un rewrite SPA de toutes les routes vers `index.html` et impose des en-têtes de sécurité, dont une **CSP stricte**. `connect-src` n'autorise que `'self'`, l'API de production (`https://site-cava.onrender.com`) et Cloudinary (`https://api.cloudinary.com`) ; `img-src`/`media-src` autorisent Cloudinary et `i.ytimg.com`. Toute nouvelle intégration tierce (autre domaine d'API, iframe, script analytics) sera bloquée tant que `vercel.json` n'est pas mis à jour en conséquence.
  - `Permissions-Policy` doit garder **`camera=(self)`** : c'est ce qui laisse le badgeage (`/presences`) ouvrir la caméra. L'ancienne valeur `camera=()` interdisait la caméra au site lui-même : Chrome — donc tout Android — rejetait `getUserMedia` (`NotAllowedError`) **sans même afficher la demande d'autorisation**, tandis que Safari ignore cet en-tête. D'où une panne « Android seulement », présente du 4 août au 11 septembre 2026, que sept correctifs successifs du composant n'avaient pas pu toucher. La portée est globale, pas limitée à `/presences` : la politique est fixée au **chargement** du document, et le site est une SPA. Si elle régresse, le scanner affiche « bloquée par la configuration du site ».
